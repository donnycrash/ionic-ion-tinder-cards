(function(ionic) {
    ...
    var SwipeableCardView = ionic.views.View.inherit({
        ...
        initialize: function(opts) {
            ...
            this.parentHeight = this.el.parentNode.parentNode.offsetHeight;
        ...
        transitionOut: function(e) {
            ...
            if (this.isUnderThreshold() && this.isUnderYThreshold()) {
                self.onSnapBack(this.x, this.y, this.rotationAngle);
                return;
            }

            self.onTransitionOut(self.thresholdAmount, self.thresholdAmountY);
            var abs = Math.abs(self.thresholdAmount);
            var absY = Math.abs(self.thresholdAmountY);
            if (abs > absY) {
                ...
                // Target Y is just the "opposite" side of the triangle of targetX as the adjacent edge (sohcahtoa yo)
                var targetX = targetY / Math.tan(angle);
        ...
        _doDrag: function(e) {
            ...
            this.thresholdAmountY = (this.y / (this.el.offsetHeight / 2));

            var self = this;
            setTimeout(function() {
                self.onPartialSwipe(self.thresholdAmount, self.thresholdAmountY);
            });
        },
        ...
        return {
            ...
            compile: function(element, attr) {
                ...
                return function($scope, $element, $attr, swipeCards) {
                    ...
                    var directions = $attr.directions;
                    var functionsRequired = [];
                    ...
                    angular.forEach($attr, function(value, key) {
                        ...
                        if (typeof(value) === 'string' && key.indexOf('onSwipe') > -1) {
                            functionsRequired.push(key);
                        };
                    });
                    ...
                    var swipeableCard = new SwipeableCardView({
                        ...
                        leftText: leftText,
                        rightText: rightText,
                        functions: functionsRequired,
                        directions: directions.split('-'),
                        onPartialSwipe: function(amt, amtY) {
                            swipeCards.partial(amt, amtY);
                            var self = this;
                            $timeout(function() {
                                if (amt < 0) {
                        ...
                        onSwiped: function(direction) {
                            ...
                            $timeout(function() {
                                $scope.onSwiped(direction);
                                if (swipeableCard.functions) {
                                    var _cards = swipeableCard.functions;
                                    for (var i = 0; i < _cards.length; i++) {
                                        if (_cards[i].toLowerCase().indexOf(direction.direction) > -1) {
                                            if (swipeableCard[_cards[i]]) {
                                                swipeableCard[_cards[i]]();
                                            } else {
                                                console.log('No Method For ' + _cards[i])
                                            }
                                        }
                                    };
                                }
                        ...
                        onTransitionOut: function(amt, amtY) {

                            var posAmt = Math.abs(amt);
                            var posAmtY = Math.abs(amtY);
                            var direction = {};
                            if (posAmt > posAmtY) {
                                if (amt < 0) {
                            ...
                            swipeableCard.onSwiped(direction);
